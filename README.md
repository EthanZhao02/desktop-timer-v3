...
| 💬 **星野对话** | 点击宠物对话，支持多模型（星野/DeepSeek/火山） |
...
点击桌面宠物星野，直接和 AI 助手对话！

### 支持的模型

| 模型 | 说明 | 配置方式 |
|------|------|----------|
| ⭐ **星野** | 本地 QClaw 助手，无需联网 | 自动连接 localhost:53717 |
| 🤖 **DeepSeek** | DeepSeek-V3 大模型 | 设置环境变量 `DEEPSEEK_API_KEY` |
| 🌋 **火山引擎** | 字节跳动豆包大模型 | 设置环境变量 `VOLCANO_API_KEY` |

### 使用方法

1. **点击宠物** → 弹出对话面板
2. **选择模型** → 顶部下拉框切换
3. **输入文字** → 按 Enter 或点击发送
4. **打开计时器** → 点击 ⏱️ 按钮

聊天记录自动保存（最近20条），可点击 🗑️ 清空。

> 💡 适合在做事的间隙快速问一句：「这个报错怎么解决？」「帮我写个正则」
